...
def precession_eqns(t, y_vec, m1, m2): 
	...
	dv_dt = -1./(m*stf2.denergy_by_flux(v, eta, delta, chiadL, chisdL, chiasqr, chissqr, chisdchia, 7))

	return  [dv_dt, dS1_dt[0], dS1_dt[1], dS1_dt[2], dS2_dt[0], dS2_dt[1], dS2_dt[2], dL_dt[0], dL_dt[1], dL_dt[2]]


def evolve_spins(v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z, Lnx, Lny, Lnz, v_final): 
	...
	dt = 1./128. #1./16. #1./64. #1./128. #1./256. #1./2048
	T_MAX = 1e7
	R_TOL = 1e-6 
	A_TOL = 1e-6 
...
def find_SdotL_at_freq(v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z, Lnx, Lny, Lnz, v_final): 
	""" given the spins and ang momentum at a given frequency, find the dot product between the spins and L at a later frequency """ 

	# evolve the spins 
	v_v, chi1x_v, chi1y_v, chi1z_v, chi2x_v, chi2y_v, chi2z_v, Lnx_v, Lny_v, Lnz_v = evolve_spins(v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z, Lnx, Lny, Lnz, v_final)
	...
	plt.subplot(311)
	plt.plot(v_v, chi1x_v, 'r', label='$x$', lw=1) 
	plt.plot(v_v, chi1y_v, 'g', label='$y$', lw=1) 
	plt.plot(v_v, chi1z_v, 'b', label='$z$', lw=1) 
	plt.plot(v_v, chi1_norm, 'm', label='norm', lw=0.5)
	plt.plot(v_v, chi1dL_v, 'k', lw=2, label='$\chi \cdot L$')
	plt.plot(v_v[-1], chi1dL_v[-1], 'o')
	plt.ylim(-1,1)
	plt.legend(frameon=False)
	plt.ylabel('$\chi_1$')
	plt.grid()
	plt.subplot(312)
	plt.plot(v_v, chi2x_v, 'r', label='x', lw=1) 
	plt.plot(v_v, chi2y_v, 'g', label='y', lw=1) 
	plt.plot(v_v, chi2z_v, 'b', label='z', lw=1) 
	plt.plot(v_v, chi2_norm, 'm', lw=0.5)
	plt.plot(v_v, chi2dL_v, 'k', lw=2)
	plt.plot(v_v[-1], chi2dL_v[-1], 'o')
	plt.ylabel('$\chi_2$')
	plt.ylim(-1,1)
	plt.grid()
	plt.subplot(313)
	plt.plot(v_v, Lnx_v, 'r', label='x', lw=1) 
	plt.plot(v_v, Lny_v, 'g', label='y', lw=1) 
	plt.plot(v_v, Lnz_v, 'b', label='z', lw=1) 
	plt.plot(v_v, L_norm, 'm', lw=0.5)
	plt.ylabel('$L_N$')
	plt.xlabel('$v$')
	plt.ylim(-1,1)
	plt.grid()
	plt.savefig('spinev.png', dpi=300)

	return chi1dL_v[-1], chi2dL_v[-1], v_v[-1]

def find_S_and_L_at_freq(v0, m1, m2, chi1x, chi1y, chi1z, chi2x, chi2y, chi2z, Lnx, Lny, Lnz, v_final):
