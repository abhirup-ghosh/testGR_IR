# Code to read in LALInference posterior samples *from two runs*, combine them, and calculate the medians (and other quantities, including histograms) of the posteriors on the ISCO frequency of the final Kerr black hole and on the peak frequency of the 2,2 mode, using the Taracchini et al. fits for the last of these. Note that we work with the redshifted masses throughout.
...
parser.add_argument("datfile1", help = "first posterior_samples.dat file to read in")
parser.add_argument("datfile2", help = "second posterior_samples.dat file to read in")
...
data1 = np.genfromtxt(args.datfile1, dtype=None, names=True)
data2 = np.genfromtxt(args.datfile2, dtype=None, names=True)
#if args.redshifted_m:
m1, m2 = np.append(data1['m1'], data2['m1']), np.append(data1['m2'], data2['m2'])
#else:
...
if ('a1z' in data1.dtype.names) and ('a2z' in data2.dtype.names):
  chi1, chi2 = np.append(data1['a1z'], data2['a1z']), np.append(data1['a2z'], data2['a2z'])
else:
