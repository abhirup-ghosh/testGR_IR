function [beta,r,J,Sigma,mse] = EOBnlinfit(X,y,model,beta,options)
...
%   $Revision: 2.22.2.16 $  $Date: 2008/12/01 08:09:22 $
...
try
    ...
    if isa(model, 'inline')
        throw(addCause(MException('stats:nlinfit:ModelFunctionError',...
                          'Error evaluating inline model function.'),ME));
    elseif strcmp('MATLAB:UndefinedFunction', ME.identifier) ...
        ...
        throw(addCause(MException('stats:nlinfit:ModelFunctionError',...
                          'Error evaluating model function ''%s''.',...
                          func2str(model)),...
                       ME));
    end
end
