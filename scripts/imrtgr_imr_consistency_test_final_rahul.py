...
"""
#def kde_pdf(data,bins): #rahul for getting Mf,af pdf
...
if __name__ == '__main__':
  ...
  if m1_inj == None or m2_inj == None or chi1_inj == None or chi2_inj == None:
    ...
    Mf_inj, chif_inj = tgr.calc_final_mass_spin(m1_inj, m2_inj, 0., 0., chi1_inj, chi2_inj, 0., 0., 0., fit_formula)
  ...
  m1_i, m2_i, chi1_i, chi2_i = insp_data['m1'], insp_data['m2'], insp_data['a1'], insp_data['a2']
  # if there is are tilt1, tilt2 and phi12 columns in the posterior, read the values.
  if ('tilt1' in insp_data.dtype.names) and ('tilt2' in insp_data.dtype.names) and ('phi12' in insp_data.dtype.names):
    tilt1_i, tilt2_i, phi12_i = insp_data['tilt1'], insp_data['tilt2'], insp_data['phi12']
  else:
    tilt1_i, tilt2_i, phi12_i = np.zeros(len(m1_i)), np.zeros(len(m1_i)), np.zeros(len(m1_i))
  if ('a1z' in insp_data.dtype.names) and ('a2z' in insp_data.dtype.names):
    chi1z_i, chi2z_i = insp_data['a1z'], insp_data['a2z']
  else:
    chi1z_i, chi2z_i = chi1_i*np.cos(tilt1_i), chi2_i*np.cos(tilt2_i)


  """
  ########## Applying elementary KDE on inspiral posterior data set ###########
  #m1_pos=np.linspace(m1_i.min(),m1_i.max(),100)
  #kernel = gaussian_kde(m1_i)
  #m1_pdf=kernel(m1_pos)
  #plt.plot(m1_pos,m1_pdf)
  #plt.hist(m1_i,normed=True,bins='auto')
  #plt.title('mass1')
  #plt.savefig('KDE_results/m1_pdf.png')
  #plt.clf()
  #chi1_pos=np.linspace(chi1_i.min(),chi1_i.max(),100)
  #kernel = gaussian_kde(chi1_i)
  #chi1_pdf = kernel(chi1_pos)
  #plt.plot(chi1_pos,chi1_pdf)
  #plt.hist(chi1_i,normed=True,bins='auto')
  #plt.title('Chi1')
  #plt.savefig('KDE_results/chi1_pdf.png')
  # comparing kde pdf with the original dataset
  """
  ## YES/NO KDE of lal results? ##
  kde_lalresults=False
  #KDE related scriptwise global parameters
  kde_sampleSize=20000; bandwidth_factor=1.

  # KDE of inspiral;
  if (kde_lalresults):
	  insp_data_old = np.vstack([m1_i, m2_i, chi1_i, chi2_i, chi1z_i, chi2z_i, tilt1_i, tilt2_i, phi12_i])
	  kernel=gaussian_kde(insp_data_old)
	  kernel.set_bandwidth(bw_method=kernel.factor/bandwidth_factor)
	  insp_data_new = kernel.resample(kde_sampleSize)
	  print 'Original LAL sample-size:',len(insp_data_old[0]),'LAL KDE-resampled-size ',len(insp_data_new[0])
	  #print 'new insp data limits before rejection:',insp_data_new[2].min(),insp_data_new[2].max(),insp_data_new[3].min(),insp_data_new[3].max()
	  keep = (0 < insp_data_new[0]); insp_data_new[0]=abs(insp_data_new[0]*keep)  #these two lines removes data from chi fields resamples
	  keep = (0 < insp_data_new[1]); insp_data_new[1]=abs(insp_data_new[1]*keep)
	  keep = (0 < insp_data_new[2]) * (insp_data_new[2] <= 0.99); insp_data_new[2]=abs(insp_data_new[2]*keep)  #these two lines removes data from chi fields resamples
	  keep = (0 < insp_data_new[3]) * (insp_data_new[3] <= 0.99); insp_data_new[3]=abs(insp_data_new[3]*keep)
	  #keep = (0 < insp_data_new[4]) * (insp_data_new[4] <= 0.99); insp_data_new[4]=abs(insp_data_new[4]*keep)  #these two lines removes data from chi fields resamples
	  #keep = (0 < insp_data_new[5]) * (insp_data_new[5] <= 0.99); insp_data_new[5]=abs(insp_data_new[5]*keep)

	  #print 'new insp data limits after rejection:',insp_data_new[2].min(),insp_data_new[2].max(),insp_data_new[3].min(),insp_data_new[3].max()
	  m1_i, m2_i, chi1_i, chi2_i, chi1z_i, chi2z_i, tilt1_i, tilt2_i, phi12_i = insp_data_new[0],insp_data_new[1],insp_data_new[2],insp_data_new[3],insp_data_new[4],insp_data_new[5],insp_data_new[6],insp_data_new[7],insp_data_new[8]


	  fields=['m1-i','m2-i','chi1-i','chi2-i','chi1z-i','chi2z-i','tilt1-i','tilt2-i','phi12-i']
	  for i in range(8):
	     plt.hist(insp_data_old[i],normed=True,bins='auto',alpha=0.8)
	     plt.hist(insp_data_new[i],normed=True,bins='auto',alpha=0.4)
	     plt.title(fields[i])
	     plt.savefig('KDE_results/insp_data_compare/'+fields[i]+'_resample.png')
	     plt.clf()

	  fields=['m1','m2','a1','a2','tilt1','tilt2','phi12','a1z','a2z']
	  for field in fields:
	      #print 'field=',field,len(insp_data[field]),insp_data[field]
	      postn=np.linspace(insp_data[field].min(),insp_data[field].max(),100)
	      kernel = gaussian_kde(insp_data[field])
	      field_pdf=kernel(postn)
	      plt.plot(postn,field_pdf,label=field)
	      plt.hist(insp_data[field],normed=True,bins='auto')
	      plt.title(field)
	      plt.savefig('KDE_results/insp_data_compare/'+field+'-i_pdf.png')
	      plt.clf()
	  """
	  #This is univariate way of doing things in KDE
	  kernel=gaussian_kde(m1_i);m1_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(m2_i);m2_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(chi1_i);chi1_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(chi2_i);chi2_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(chi1z_i);chi1z_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(chi2z_i);chi2z_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(tilt1_i);tilt1_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(tilt2_i);tilt2_i=kernel.resample(kde_sampleSize)
	  kernel=gaussian_kde(phi12_i);phi12_i=kernel.resample(kde_sampleSize)
	  """
	  #print chi1_i, chi2_i
	  #print 'chi1 and chi2 values limits going into tgr.calc_final_mass_spin:',chi1_i.min(),chi1_i.max(),chi2_i.min(),chi2_i.max()
	  #print 'chi1 outside imrtgrutils',chi1_i
	  #print 'chi2 outside imrtgrutils',chi2_i
  # compute the final mass and spin
  Mf_i, chif_i = tgr.calc_final_mass_spin(m1_i, m2_i, chi1_i, chi2_i, chi1z_i, chi2z_i, tilt1_i, tilt2_i, phi12_i, fit_formula)
  chi1_i, chi2_i = chi1z_i, chi2z_i


  # read data from the ringdown posterior file
  ring_data = np.genfromtxt(ring_post, dtype=None, names=True)
  m1_r, m2_r, chi1_r, chi2_r = ring_data['m1'], ring_data['m2'], ring_data['a1'], ring_data['a2']
  # if there is are tilt1, tilt2 and phi12 columns in the posterior, read the values.
  if ('tilt1' in ring_data.dtype.names) and ('tilt2' in ring_data.dtype.names) and ('phi12' in ring_data.dtype.names):
    tilt1_r, tilt2_r, phi12_r = ring_data['tilt1'], ring_data['tilt2'], ring_data['phi12']
  else:
    tilt1_r, tilt2_r, phi12_r = np.zeros(len(m1_r)), np.zeros(len(m1_r)), np.zeros(len(m1_r))
  if ('a1z' in ring_data.dtype.names) and ('a2z' in ring_data.dtype.names):
    chi1z_r, chi2z_r = ring_data['a1z'], ring_data['a2z']
  else:
    chi1z_r, chi2z_r = chi1_r*np.cos(tilt1_r), chi2_r*np.cos(tilt2_r)

  # KDE of ringdown;
  if (kde_lalresults):
	  ring_data_old = np.vstack([m1_r, m2_r, chi1_r, chi2_r, chi1z_r, chi2z_r, tilt1_r, tilt2_r, phi12_r])
	  kernel=gaussian_kde(ring_data_old)
	  kernel.set_bandwidth(bw_method=kernel.factor/bandwidth_factor)
	  ring_data_new = kernel.resample(kde_sampleSize)
	  keep = (0 < ring_data_new[0]); ring_data_new[0]=abs(ring_data_new[0]*keep)
	  keep = (0 < ring_data_new[1]); ring_data_new[1]=abs(ring_data_new[1]*keep)
	  keep = (0 < ring_data_new[2]) * (ring_data_new[2] <= 0.99); ring_data_new[2]=abs(ring_data_new[2]*keep)  #these two lines removes data from chi resamples
	  keep = (0 < ring_data_new[3]) * (ring_data_new[3] <= 0.99); ring_data_new[3]=abs(ring_data_new[3]*keep)
	  m1_r, m2_r, chi1_r, chi2_r, chi1z_r, chi2z_r, tilt1_r, tilt2_r, phi12_r = ring_data_new[0],ring_data_new[1],ring_data_new[2],ring_data_new[3],ring_data_new[4],ring_data_new[5],ring_data_new[6],ring_data_new[7],ring_data_new[8]


	  fields=['m1-r','m2-r','chi1-r','chi2-r','chi1z-r','chi2z-r','tilt1-r','tilt2-r','phi12-r']
	  for i in range(8):
	     plt.hist(ring_data_old[i],normed=True,bins='auto',alpha=0.8)
	     plt.hist(ring_data_new[i],normed=True,bins='auto',alpha=0.4)
	     plt.title(fields[i])
	     plt.savefig('KDE_results/ring_data_compare/'+fields[i]+'_resample.png')
	     plt.clf()

	  fields=['m1','m2','a1','a2','tilt1','tilt2','phi12','a1z','a2z']
	  for field in fields:
	      #print 'field=',field,len(ring_data[field]),ring_data[field]
	      postn=np.linspace(ring_data[field].min(),ring_data[field].max(),100)
	      kernel = gaussian_kde(ring_data[field])
	      field_pdf=kernel(postn)
	      plt.plot(postn,field_pdf,label=field)
	      plt.hist(ring_data[field],normed=True,bins='auto')
	      plt.title(field)
	      plt.savefig('KDE_results/ring_data_compare/'+field+'-i_pdf.png')
	      plt.clf()

  # compute the final mass and spin
  Mf_r, chif_r = tgr.calc_final_mass_spin(m1_r, m2_r, chi1_r, chi2_r, chi1z_r, chi2z_r, tilt1_r, tilt2_r, phi12_r, fit_formula)
  chi1_r, chi2_r = chi1z_r, chi2z_r


  # read data from the IMR posterior file
  imr_data = np.genfromtxt(imr_post, dtype=None, names=True)
  m1_imr, m2_imr, chi1_imr, chi2_imr = imr_data['m1'], imr_data['m2'], imr_data['a1'], imr_data['a2']
  # if there is are tilt1, tilt2 and phi12 columns in the posterior, read the values.
  if ('tilt1' in imr_data.dtype.names) and ('tilt2' in imr_data.dtype.names) and ('phi12' in imr_data.dtype.names):
    tilt1_imr, tilt2_imr, phi12_imr = imr_data['tilt1'], imr_data['tilt2'], imr_data['phi12']
  else:
    tilt1_imr, tilt2_imr, phi12_imr = np.zeros(len(m1_imr)), np.zeros(len(m1_imr)), np.zeros(len(m1_imr))
  if ('a1z' in imr_data.dtype.names) and ('a2z' in imr_data.dtype.names):
    chi1z_imr, chi2z_imr = imr_data['a1z'], imr_data['a2z']
  else:
    chi1z_imr, chi2z_imr = chi1_imr*np.cos(tilt1_imr), chi2_imr*np.cos(tilt2_imr)

  # KDE of IMR;
  if (kde_lalresults):
	  imr_data_old = np.vstack([m1_imr, m2_imr, chi1_imr, chi2_imr, chi1z_imr, chi2z_imr, tilt1_imr, tilt2_imr, phi12_imr])
	  kernel=gaussian_kde(imr_data_old)
	  kernel.set_bandwidth(bw_method=kernel.factor/bandwidth_factor)
	  imr_data_new = kernel.resample(kde_sampleSize)
	  keep = (0 < imr_data_new[0]); imr_data_new[0]=abs(imr_data_new[0]*keep)
	  keep = (0 < imr_data_new[1]); imr_data_new[1]=abs(imr_data_new[1]*keep)
	  keep = (0 < imr_data_new[2]) * (imr_data_new[2] <= 0.99); imr_data_new[2]=abs(imr_data_new[2]*keep)  #these two lines removes data from chi resamples
	  keep = (0 < imr_data_new[3]) * (imr_data_new[3] <= 0.99); imr_data_new[3]=abs(imr_data_new[3]*keep)
	  m1_imr, m2_imr, chi1_imr, chi2_imr, chi1z_imr, chi2z_imr, tilt1_imr, tilt2_imr, phi12_imr = imr_data_new[0],imr_data_new[1],imr_data_new[2],imr_data_new[3],imr_data_new[4],imr_data_new[5],imr_data_new[6],imr_data_new[7],imr_data_new[8]


	  fields=['m1-imr','m2-imr','chi1-imr','chi2-imr','chi1z-imr','chi2z-imr','tilt1-imr','tilt2-imr','phi12-imr']
	  for i in range(8):
	     plt.hist(imr_data_old[i],normed=True,bins='auto',alpha=0.8)
	     plt.hist(imr_data_new[i],normed=True,bins='auto',alpha=0.4)
	     plt.title(fields[i])
	     plt.savefig('KDE_results/imr_data_compare/'+fields[i]+'_resample.png')
	     plt.clf()

	  fields=['m1','m2','a1','a2','tilt1','tilt2','phi12','a1z','a2z']
	  for field in fields:
	      #print 'field=',field,len(imr_data[field]),imr_data[field]
	      postn=np.linspace(imr_data[field].min(),imr_data[field].max(),100)
	      kernel = gaussian_kde(imr_data[field])
	      field_pdf=kernel(postn)
	      plt.plot(postn,field_pdf,label=field)
	      plt.hist(imr_data[field],normed=True,bins='auto')
	      plt.title(field)
	      plt.savefig('KDE_results/imr_data_compare/'+field+'-i_pdf.png')
	      plt.clf()

  # compute the final mass and spin
  Mf_imr, chif_imr = tgr.calc_final_mass_spin(m1_imr, m2_imr, chi1_imr, chi2_imr, chi1z_imr, chi2z_imr, tilt1_imr, tilt2_imr, phi12_imr, fit_formula)
  chi1_imr, chi2_imr = chi1z_imr, chi2z_imr


  print '... read posteriors'
  ###############################################################################################
  """
  #8.Oct.2017/rahul: Calculate Joint pdf, redefine variables over nonuniform grid; do kde.resample find marginalized pdf to get P(\epsilon,\sigma)
  joint_data=np.vstack([Mf_i,Mf_r,chif_i,chif_r])
  np.save("joint_data.npy",joint_data)
  print "saved pickle file"
  kernel=stats.gaussian_kde(joint_data)
  Mf_I,Mf_R,chif_I,chif_R=np.mgrid[Mf_i.min:Mf_i.max:100j,Mf_r.min:Mf_r.max:100j,chif_i.min():chif_i.max():100j,chif_r.min():chif_r.max():100j]
  f1 = lambda x,y,z,p:kernel.evaluate([x,y,z,p])
  Z=np.vectorize(f1)(Mf_I,Mf_R,chif_I,chif_R)
  #8.Oct.2017/rahul: For some other time
  """
  ###############################################################################################
  ...
  if MfafKDE:
    ...
    joint_data=np.vstack([Mf_i,chif_i]);kernel_i=gaussian_kde(joint_data)
    f_i = lambda x,y:kernel_i.evaluate([x,y])
    print "for inspiral kernel",kernel_i.integrate_box([-Mf_lim,-chif_lim],[Mf_lim,chif_lim])
    P_Mfchif_i=np.vectorize(f_i)(M_i,C_i)/kernel_i.integrate_box([-Mf_lim,-chif_lim],[Mf_lim,chif_lim])

    joint_data=np.vstack([Mf_r,chif_r]);kernel_r=gaussian_kde(joint_data)#;M_i,C_i=np.meshgrid(Mf_bins,chif_bins)
    f_r = lambda x,y:kernel_r.evaluate([x,y])
    print "for post-inspiral kernel",kernel_r.integrate_box([-Mf_lim,-chif_lim],[Mf_lim,chif_lim])
    P_Mfchif_r=np.vectorize(f_r)(M_i,C_i)/kernel_r.integrate_box([-Mf_lim,-chif_lim],[Mf_lim,chif_lim])

    joint_data=np.vstack([Mf_imr,chif_imr]);kernel_imr=gaussian_kde(joint_data)#;M_i,C_i=np.meshgrid(Mf_bins,chif_bins)
    f_imr = lambda x,y:kernel_imr.evaluate([x,y])
    print "for imr kernel",kernel_imr.integrate_box([-Mf_lim,-chif_lim],[Mf_lim,chif_lim])
    P_Mfchif_imr=np.vectorize(f_imr)(M_i,C_i)/kernel_imr.integrate_box([-Mf_lim,-chif_lim],[Mf_lim,chif_lim])
  ...
  dchifbychif_vec = np.linspace(-2.0, 2.0, N_bins)
  ...
  plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_Mfchif_i),  linewidths=(1,1.5)) #rahul: switched levels:levels=(s2_Mfchif_i,s1_Mfchif_i)
  if plot_injection_lines == True:
  ...
  plt.contour(m1_bins_r[:-1], m2_bins_r[:-1], tgr.gf(P_m1m2_r),  linewidths=(1,1.5)) #rahul: levels=(s2_m1m2_r,s1_m1m2_r),
  if plot_injection_lines == True:
    plt.axvline(x=m1_inj, ls='--', color='k')
    plt.axhline(y=m2_inj, ls='--', color='k')
  plt.xlabel('$m_1 [M_{\odot}]$')
