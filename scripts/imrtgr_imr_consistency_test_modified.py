...
import time, os 
import lalinference.imrtgr.imrtgrutils_modified as tgr
import pickle, gzip
import sys 
...
if __name__ == '__main__':
  ...
  P_dMfbyMf_daf = np.zeros(shape=(N_bins,N_bins))

  for i, v2 in enumerate(daf_vec):
    for j, v1 in enumerate(dMfbyMf_vec):
      P_dMfbyMf_daf[i,j] = tgr.calc_sum(Mf_intp, af_intp, v1, v2, P_dMfdaf_interp_object, P_Mfaf_imr_interp_object)*dx*dy
  
  # normalization
  P_dMfbyMf_daf /= np.sum(P_dMfbyMf_daf) * dx * dy
  ...
  # GR confidence
  conf_v1v2 = confidence(P_dMfbyMf_daf)
  gr_height = P_dMfbyMf_daf[np.argmin(abs(dMfbyMf_vec)), np.argmin(abs(daf_vec))]
  ...
  np.savetxt(out_dir+'/data/daf_vec.dat', daf_vec)
  np.savetxt(out_dir+'/data/P_dMfbyMf_daf.dat', P_dMfbyMf_daf)
  np.savetxt(out_dir+'/data/P_dMfbyMf.dat', P_dMfbyMf)
  np.savetxt(out_dir+'/data/P_daf.dat', P_daf)
  ...
  left1_v2 = min(daf_vec[np.where(P_daf>s1_v2)[0]])
  right1_v2 = max(daf_vec[np.where(P_daf>s1_v2)[0]])

  left2_v2 = min(daf_vec[np.where(P_daf>s2_v2)[0]])
  right2_v2 = max(daf_vec[np.where(P_daf>s2_v2)[0]])
  ...
  plt.contour(dMfbyMf_vec,daf_vec,tgr.gf(P_dMfbyMf_daf), levels=(s1_v1v2,s2_v1v2), linewidths=(1,1.5))
  plt.plot(0, 0, 'k+', ms=12, mew=2)
  plt.xlabel('$\Delta M_f/M_f$')
  plt.ylabel('$\Delta a_f$')
  plt.xlim([-1.,1.]) 
