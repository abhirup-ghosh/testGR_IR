...
mf_max = 2.*max(abs(min(mf_imr)), abs(max(mf_imr)))#+10.
af_max = 2.*max(abs(min(af_imr)), abs(max(af_imr)))#+0.1
...
# save results 
np.savetxt(out_dir+'/v1v2_imr_lim.dat', (v1_an,v2_an))
np.savetxt(out_dir+'/P_v1v2_imr_lim.dat', P_v1v2_intrp)
#########################################################################################
...
plt.subplot(235)
plt.pcolormesh(v1_an,v2_an,gf(P_v1v2_intrp).T, cmap='YlOrBr')
plt.colorbar()
plt.contour(v1_an, v2_an, gf(P_v1v2_intrp).T, levels=(s1_v1v2,s2_v1v2), linewidth=0.1, cmap='hot')
plt.axvline(x=0, ls='--', lw=1, color='k')
plt.axhline(y=0, ls='--', lw=1, color='k')
