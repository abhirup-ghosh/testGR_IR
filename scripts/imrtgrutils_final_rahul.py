...
def calc_final_mass_spin(m1, m2, chi1, chi2, chi1z, chi2z, tilt1, tilt2, phi12, fit_formula):
  #print 'chi1 and chi2 values limits inside tgr.calc_final_mass_spin:',chi1.min(),chi1.max(),chi2.min(),chi2.max()  #rahul
  print 'chi1 inside imrtgrutils',chi1
  print 'chi2 inside imrtgrutils',chi2
...
def calc_Mfchif_prior_samples(comp_mass_prior_min, comp_mass_prior_max, comp_spin_min, comp_spin_max, Mf_bins, chif_bins, fit_formula, spin_angle_dist, N_sampl, thread):
  ...
  return calc_final_mass_spin(m1_pr, m2_pr, np.zeros(len(m1_pr)), np.zeros(len(m1_pr)), chi1_pr, chi2_pr, np.zeros(len(m1_pr)), np.zeros(len(m1_pr)), np.zeros(len(m1_pr)), fit_formula)
