...
black hole in a binary black hole merger. This script uses the following definitions for the fractional 
parameters describing deviations from GR

epsilon = 1-Mf^I/Mf^R, 	sigma = 1-af^I/af^R 
...
if __name__ == '__main__':
  ...
  Mf_lim = 2*max(abs(np.append(np.append(Mf_i, Mf_r), Mf_imr)))
  chif_lim = 2*max(abs(np.append(np.append(chif_i, chif_r), chif_imr)))
  ...
  ################################################################################################
  # compute the posterior of (1- Mf^R/Mf^I, 1 - chif^R/chif^I)
  ################################################################################################

  # Obtaining interpolating objects for the inspiral and post-inspiral Mf, af posteriors
  ...
  # defining limits of MfRbyMfI_vec and chifRbychifI_vec
  MfRbyMfI_vec = np.linspace(5., 0., N_bins)
  chifRbychifI_vec = np.linspace(5., 0., N_bins)
  P_MfRbyMfI_chifRbychifI  = np.ones(shape=(N_bins,N_bins))

	# compute the posterior P_mx(Mf^R/Mf^I, chif^R/chif^I)
  for i, v2 in enumerate(chifRbychifI_vec):
    for j, v1 in enumerate(MfRbyMfI_vec):
			P_MfRbyMfI_chifRbychifI[i,j] = tgr.calc_sum(Mf_intp, chif_intp, v1, v2, P_Mfchif_r_interp_object, P_Mfchif_i_interp_object)

  conf_v1v2 = confidence(P_MfRbyMfI_chifRbychifI)
  gr_height = P_MfRbyMfI_chifRbychifI[np.argmin(abs(MfRbyMfI_vec-1)),np.argmin(abs(chifRbychifI_vec-1))] # taking value closest to the GR value (1,1)
  gr_conf_level = conf_v1v2.level_from_height(gr_height)
  print '... From (delta_Mf/Mf_i, delta_af/af_i): no deviation from GR above %.1f%% confidence level (epsilon, sigma)'%(100.*gr_conf_level)

  # save results 
  np.savetxt(out_dir+'/data/P_MfRbyMfI_chifRbychifI.dat.gz', P_MfRbyMfI_chifRbychifI)
  np.savetxt(out_dir+'/data/MfRbyMfI_vec.dat.gz', MfRbyMfI_vec)
  np.savetxt(out_dir+'/data/chifRbychifI_vec.dat.gz', chifRbychifI_vec)
  np.savetxt(out_dir+'/data/GR_confidence_inspnorm.txt', [gr_conf_level])

  conf_eps_sig = confidence(P_MfRbyMfI_chifRbychifI)
  s1_eps_sig = conf_eps_sig.height_from_level(0.68)
  s2_eps_sig = conf_eps_sig.height_from_level(0.95)

	# plot posterior of (1-Mf^I/Mf^R, 1-af^I/af^R)
  plt.figure(figsize=(5,5))
  plt.pcolormesh(1-MfRbyMfI_vec, 1-chifRbychifI_vec, tgr.gf(P_MfRbyMfI_chifRbychifI), cmap='YlOrBr')
  plt.contour(1-MfRbyMfI_vec, 1-chifRbychifI_vec, tgr.gf(P_MfRbyMfI_chifRbychifI), levels=(s2_eps_sig,s1_eps_sig), linewidths=(1,1.5))
  plt.plot(0, 0, 'k+', ms=12, mew=2)
  plt.xlabel('$1 - M_f^R/M_f^I$')
  plt.ylabel('$1 -a_f^R/a_f^I$')
  plt.grid()
  plt.tight_layout()
  plt.savefig('%s/img/P_1minus_MfIbyMfR_1minus_afIbyafR_thumb.png'%(out_dir), dpi=72)
  plt.savefig('%s/img/P_1minus_MfIbyMfR_1minus_afIbyafR.png'%(out_dir), dpi=300)
  plt.close()

	# scatter plot of (1-Mf^R/Mf^I, 1-af^R/af^I)
  plt.figure(figsize=(5,5))
  N_sampl = min([len(Mf_i), len(Mf_r)])
  plt.plot(1-Mf_r[:N_sampl]/Mf_i[:N_sampl], 1-chif_r[:N_sampl]/chif_i[:N_sampl], 'k.', ms=0.1)
  ...
  plt.grid()
  plt.tight_layout()
  plt.savefig('%s/img/1minus_MfIbyMfR_1minus_afIbyafR_scatter_thumb.png'%(out_dir), dpi=72)
  plt.savefig('%s/img/1minus_MfIbyMfR_1minus_afIbyafR_scatter.png'%(out_dir), dpi=300)
  plt.close()
  ...
  conf_v1v2 = confidence(P_dMfdchif)
  gr_height = P_dMfdchif[np.argmin(abs(Mf_intp)), np.argmin(abs(chif_intp))] # taking value closest to (0,0)
  gr_conf_level = conf_v1v2.level_from_height(gr_height)
  print '... From delta: no deviation from GR above %.1f%% confidence level'%(100.*gr_conf_level)

  np.savetxt(out_dir+'/data/GR_confidence_nonorm.txt', [gr_conf_level])
  print '... computed P(delta_Mf, delta_chif)'
	
  # scatter plot of (1-Mf^R/Mf^I, 1-af^R/af^I)
  plt.figure(figsize=(5,5))
  N_sampl = min([len(Mf_i), len(Mf_r)])
  plt.plot(Mf_i[:N_sampl]-Mf_r[:N_sampl], chif_i[:N_sampl]-chif_r[:N_sampl], 'k.', ms=0.1)
  if plot_injection_lines == True:
    plt.axvline(x=0., ls='--', color='k')
    plt.axhline(y=0., ls='--', color='k')
  ...
  plt.grid()
  plt.tight_layout()
  plt.savefig('%s/img/dMfdchif_scatter_thumb.png'%(out_dir), dpi=72)
  plt.savefig('%s/img/dMfdchif_scatter.png'%(out_dir), dpi=300)
  plt.close()
  ###############################################################################################

  ################################################################################################
  # compute the posterior of (delta_Mf/Mf_mean, delta_chif/chif_mean)
  ################################################################################################
  P_Mfmean_chifmean = dMf*dchif*ss.convolve2d(P_Mfchif_i, P_Mfchif_r, boundary='fill', mode='same')

  # (mean_Mf, mean_chif)
  conf_dMfdchif = confidence(P_Mfmean_chifmean)
  s1_Mfmean_chifmean = conf_dMfdchif.height_from_level(0.68)
  s2_Mfmean_chifmean = conf_dMfdchif.height_from_level(0.95)

  plt.figure(figsize=(5,5))
  plt.pcolormesh(0.5*Mf_bins, 0.5*chif_bins, tgr.gf(P_Mfmean_chifmean), cmap='YlOrBr')
  plt.contour(0.5*Mf_bins[:-1], 0.5*chif_bins[:-1], tgr.gf(P_Mfmean_chifmean), levels=(s2_Mfmean_chifmean,s1_Mfmean_chifmean), linewidths=(1,1.5))
  if plot_injection_lines == True:
  	plt.plot(Mf_inj, chif_inj, 'k+', ms=12, mew=2)
  plt.xlabel('$\\bar M_f~[M_\odot]$')
  plt.ylabel('$\\bar a_f$')
  plt.grid()
  plt.savefig('%s/img/P_Mfmean_chifmean.png'%(out_dir), dpi=300)
  plt.savefig('%s/img/P_Mfmean_chifmean_thumb.png'%(out_dir), dpi=72)
  plt.close()

  # scatter plot of (1-Mf^R/Mf^I, 1-af^R/af^I)
  plt.figure(figsize=(5,5))
  N_sampl = min([len(Mf_i), len(Mf_r)])
  plt.plot(0.5*(Mf_i[:N_sampl]+Mf_r[:N_sampl]), 0.5*(chif_i[:N_sampl]+chif_r[:N_sampl]), 'k.', ms=0.1)
  if plot_injection_lines == True:
    plt.axvline(x=Mf_inj, ls='--', color='k')
    plt.axhline(y=Mf_inj, ls='--', color='k')
  plt.xlabel('$\\bar M_f~[M_\odot]$')
  plt.ylabel('$\\bar a_f$')
  plt.grid()
  plt.tight_layout()
  plt.savefig('%s/img/Mfmean_chifmean_scatter_thumb.png'%(out_dir), dpi=72)
  plt.savefig('%s/img/Mfmean_chifmean_scatter.png'%(out_dir), dpi=300)
  plt.close()
  ...
  P_Mfmean_chifmean_interp_object = scipy.interpolate.interp2d(0.5*Mf_intp, 0.5*chif_intp, P_Mfmean_chifmean, fill_value=0., bounds_error=False)
  ...
  # compute the P(dMf/Mf_mean, dchif/chif_mean) by evaluating the integral 
  diff_dMfbyMf = np.mean(np.diff(dMfbyMf_vec))
  diff_dchifbychif = np.mean(np.diff(dchifbychif_vec))
  P_dMfbyMfmean_dchifbychifmean = np.zeros(shape=(N_bins,N_bins))
  ...
  for i, v2 in enumerate(dchifbychif_vec):
    for j, v1 in enumerate(dMfbyMf_vec):
      P_dMfbyMfmean_dchifbychifmean[i,j] = tgr.calc_sum(Mf_intp, chif_intp, v1, v2, P_dMfdchif_interp_object, P_Mfmean_chifmean_interp_object)

  # normalization
  P_dMfbyMfmean_dchifbychifmean /= np.sum(P_dMfbyMfmean_dchifbychifmean) * diff_dMfbyMf * diff_dchifbychif

  # Marginalization to one-dimensional joint_posteriors
  P_dMfbyMfmean = np.sum(P_dMfbyMfmean_dchifbychifmean, axis=0) * diff_dchifbychif
  P_dchifbychifmean = np.sum(P_dMfbyMfmean_dchifbychifmean, axis=1) * diff_dMfbyMf
  ...
  conf_v1v2 = confidence(P_dMfbyMfmean_dchifbychifmean)
  gr_height = P_dMfbyMfmean_dchifbychifmean[np.argmin(abs(dMfbyMf_vec)), np.argmin(abs(dchifbychif_vec))] # taking value closest to (0,0)
  gr_conf_level_normbymean = conf_v1v2.level_from_height(gr_height)
  print '... From (delta_Mf/Mf_mean, delta_af/af_mean): no deviation from GR above %.1f%% confidence level'%(100.*gr_conf_level)

  np.savetxt(out_dir+'/data/P_dMfbyMfmean_dchifbychifmean.dat.gz', P_dMfbyMfmean_dchifbychifmean)
  np.savetxt(out_dir+'/data/GR_confidence_normbymean.txt', [gr_conf_level])

	# make plots 

  # (dMf/mean_Mf, dchif/mean_chif)
  conf_v1v2 = confidence(P_dMfbyMfmean_dchifbychifmean)
  s1_v1v2 = conf_v1v2.height_from_level(0.68)
  s2_v1v2 = conf_v1v2.height_from_level(0.95)

  plt.figure(figsize=(5,5))
  plt.pcolormesh(dMfbyMf_vec,dchifbychif_vec,P_dMfbyMfmean_dchifbychifmean, cmap='YlOrBr')
  plt.contour(dMfbyMf_vec,dchifbychif_vec,tgr.gf(P_dMfbyMfmean_dchifbychifmean), levels=(s2_v1v2,s1_v1v2), linewidths=(1,1.5))
  plt.plot(0, 0, 'k+', ms=12, mew=2)
  plt.xlabel('$\Delta M_f/\\bar{M_f}$')
  plt.ylabel('$\Delta a_f/\\bar{a_f}$')
  plt.xlim([-1.,1.])
  plt.ylim([-1.,1.])
  plt.grid()
  plt.tight_layout()
  plt.savefig('%s/img/P_dMfbyMfmean_dchifbychifmean_thumb.png'%(out_dir), dpi=72)
  plt.savefig('%s/img/P_dMfbyMfmean_dchifbychifmean.png'%(out_dir), dpi=300)
  plt.close()
  ...
  gr_conf_level = conf_v1v2.level_from_height(gr_height)
  print '... From (delta_Mf/Mf_imr, delta_af/af_imr): no deviation from GR above %.1f%% confidence level'%(100.*gr_conf_level)
  ...
  plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_dMfdchif), levels=(s2_dMfdchif,s1_dMfdchif), linewidths=(1,1.5))
  plt.plot(0, 0, 'k+', ms=12, mew=2)
  plt.xlabel('$\Delta M_f~[M_\odot]$')
