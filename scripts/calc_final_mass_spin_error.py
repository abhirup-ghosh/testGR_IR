...
for i in range(len(q)): 
	...
	if abs(chi1_x[i]) < 1e-3 and abs(chi2_x[i]) < 1e-3:
		chi1dL[i], chi2dL[i], v_f[i] = chi1_z[i], chi2_z[i], v_final
	...
	print 'v_f = %f: chi1dL = %f chi2dL = %f ' %(v_f[i], chi1dL[i], chi2dL[i])
...
for err_idx in err_idx_vec: 
	print 'tag = %s q = %2.1f chi1 = [%2.1f, %2.1f, %2.1f] chi2 = [%2.1f, %2.1f, %2.1f] Mf = %f Mf_fit = %f af = %f af_fit = %f' %(SXS_run_tag[err_idx], q[err_idx], chi1_x[err_idx], chi1_y[err_idx], chi1_z[err_idx], chi2_x[err_idx], chi2_y[err_idx], chi2_z[err_idx], Mf[err_idx], Mf_fit[err_idx], af[err_idx], af_fit[err_idx])
