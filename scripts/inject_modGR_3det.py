...
if __name__ == '__main__':
  ...
  np.savetxt('%s/%s_src.dat'%(out_folder, frame_extn), np.c_[t_arr, hp_arr, hc_arr], header="t, hp, hc")
  ...
  for det in ['H1', 'L1', 'V1']:
    ...
    np.savetxt('%s/%s_strain_%s.dat'%(out_folder, frame_extn, det), np.c_[t_arr_padded, strain_t], header="t_padded, strain_t")
  ...
  print 'Network optimal SNR = %.2f'%(np.sqrt(H1_snr_opt**2 + L1_snr_opt**2 + V1_snr_opt**2))
  ...
  if plotting:
    ...
    if not add_noise:
      colors = {'H1': 'r', 'L1': 'c', 'V1': 'orange'}
    else:
      colors = {'H1': 'b', 'L1': 'g', 'V1': 'y'}
    ...
    for det in ['H1', 'L1', 'V1']:
      ...
      np.savetxt('%s/%s_frdata.dat'%(out_folder, frame_filename.replace('.gwf', '')), np.c_[frdata], header="frame_data")
      np.savetxt('%s/%s_xspacing.dat'%(out_folder, frame_filename.replace('.gwf', '')), np.c_[xspacing], header="x_spacing")
      print gps_start, xoffset, xspacing, xunit, yunit
