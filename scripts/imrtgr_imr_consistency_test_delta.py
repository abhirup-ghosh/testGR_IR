...
if __name__ == '__main__':
  ...
  MfRbyMfI_vec = np.linspace(-1.0, 1.0, N_bins)
  chifRbychifI_vec = np.linspace(-1.0, 1.0, N_bins)
  P_eps_sig = np.ones(shape=(N_bins,N_bins))

  for i, v2 in enumerate(chifRbychifI_vec):
    for j, v1 in enumerate(MfRbyMfI_vec):
			Mf_mat, chif_mat = np.meshgrid(Mf_intp, chif_intp)

			# Create diffMf and diffchif vectors corresponding to the given v1 and v2. These vectors have to be 
			# monotonically increasing in order to evaluate the interpolated prob densities. Hence, for 
			# v1, v2 > 1, ie, (1 - v1), (1 - v2) < 0, flip them, evaluate the prob density (in column or row)
			# and flip it back
			diffMf = (1-v1)*Mf_intp
			diffchif = (1-v2)*chif_intp
			if v1 > 1.:
				diffMf = np.flipud(diffMf)
			if v2 > 1.:
				diffchif = np.flipud(diffchif)

			P_r = P_Mfchif_r_interp_object(diffMf, diffchif)
			...
			P_i = P_Mfchif_i_interp_object(Mf_intp, chif_intp)
			P_intg = P_i*P_r*abs(Mf_mat*chif_mat)
			P_eps_sig[i, j] = np.sum(P_intg)*dMf*dchif

  conf_v1v2 = confidence(P_eps_sig)
  gr_height = P_eps_sig[np.argmin(abs(Mf_intp)), np.argmin(abs(chif_intp))] # taking value closest to (0,0)
  gr_conf_level = conf_v1v2.level_from_height(gr_height)
  print '... no deviation from GR above %.1f%% confidence level (epsilon, sigma)'%(100.*gr_conf_level)
  ...
  conf_eps_sig = confidence(P_eps_sig)
  s1_eps_sig = conf_eps_sig.height_from_level(0.68)
  s2_eps_sig = conf_eps_sig.height_from_level(0.95)

  plt.figure(figsize=(5,5))
  plt.pcolormesh(MfRbyMfI_vec, chifRbychifI_vec, tgr.gf(P_eps_sig), cmap='YlOrBr')
  plt.contour(MfRbyMfI_vec, chifRbychifI_vec, tgr.gf(P_eps_sig), levels=(s1_eps_sig, ), linewidths=(1,1.5))
  plt.plot(0, 0, 'k+', ms=12, mew=2)
  plt.xlabel('$1 - M_f^R/M_f^I$')
  plt.ylabel('$1 -a_f^R/a_f^I$')
  plt.grid()
  plt.savefig('%s/img/eps_sig.png'%(out_dir), dpi=300)
  plt.savefig('%s/img/eps_sig_thumb.png'%(out_dir), dpi=72)
