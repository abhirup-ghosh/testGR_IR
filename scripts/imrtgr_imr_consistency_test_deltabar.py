...
if __name__ == '__main__':
  ...
  N_bins = int(options.N_bins) # Number of grid points along either axis (eps, sig) for computation of the posteriors
  ...
  ################################################################################################
  # compute the posterior of (Mf_bar, chif_bar)
  ################################################################################################
  P_Mfbarchifbar = dMf*dchif*ss.convolve2d(P_Mfchif_i, P_Mfchif_r, boundary='fill', mode='same')

  print '... computed P(sigma_Mf, sigma_chif)'
  ###############################################################################################

  ################################################################################################
  # compute the posterior of (delta_Mf/Mf_bar, delta_chif/chif_bar)
  ################################################################################################
  ...
  P_Mfbarchifbar_interp_object = scipy.interpolate.interp2d(Mf_intp, chif_intp, P_Mfbarchifbar, fill_value=0., bounds_error=False)

  # defining limits of delta_Mf/Mf and delta_chif/chif.
  eps_vec = np.linspace(-1.0, 1.0, N_bins)
  sig_vec = np.linspace(-1.0, 1.0, N_bins)

  # compute the P(dMf/Mf, dchif/chif) by evaluating the integral 
  diff_eps = np.mean(np.diff(eps_vec))
  diff_sig = np.mean(np.diff(sig_vec))
  P_eps_sig = np.zeros(shape=(N_bins,N_bins))
  ...
  for i, v2 in enumerate(sig_vec):
    for j, v1 in enumerate(eps_vec):
      P_eps_sig[i,j] = tgr.calc_sum(Mf_intp, chif_intp, v1, v2, P_dMfdchif_interp_object, P_Mfbarchifbar_interp_object)*diff_eps*diff_sig

  # normalization
  P_eps_sig /= np.sum(P_eps_sig) * diff_eps * diff_sig

  # Marginalization to one-dimensional joint_posteriors
  P_eps = np.sum(P_eps_sig, axis=0) * diff_sig
  P_sig = np.sum(P_eps_sig, axis=1) * diff_eps
  ...
  conf_v1v2 = confidence(P_eps_sig)
  gr_height = P_eps_sig[np.argmin(abs(eps_vec)), np.argmin(abs(sig_vec))] # taking value closest to (0,0)
  ...
  np.savetxt(out_dir+'/data/eps_vec.dat.gz', eps_vec)
  np.savetxt(out_dir+'/data/sig_vec.dat.gz', sig_vec)
  np.savetxt(out_dir+'/data/P_eps_sig.dat.gz', P_eps_sig)
  np.savetxt(out_dir+'/data/P_eps.dat.gz', P_eps)
  np.savetxt(out_dir+'/data/P_sig.dat.gz', P_sig)
  ...
  plt.contour(m1_bins_i[:-1], m2_bins_i[:-1], tgr.gf(P_m1m2_i), levels=(s1_m1m2_i,), linewidths=(1,1.5))
  ...
  plt.contour(chi1_bins_i[:-1], chi2_bins_i[:-1], tgr.gf(P_chi1chi2_i), levels=(s1_chi1chi2_i,), linewidths=(1,1.5))
  if plot_injection_lines == True:
  ...
  plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_Mfchif_i), levels=(s1_Mfchif_i,), linewidths=(1,1.5))
  if plot_injection_lines == True:
  ...
  plt.contour(m1_bins_r[:-1], m2_bins_r[:-1], tgr.gf(P_m1m2_r), levels=(s1_m1m2_r,), linewidths=(1,1.5))
  if plot_injection_lines == True:
    plt.axvline(x=m1_inj, ls='--', color='k')
    plt.axhline(y=m2_inj, ls='--', color='k')
  plt.xlabel('$m_1 [M_{\odot}]$')
  ...
  plt.contour(chi1_bins_r[:-1], chi2_bins_r[:-1], tgr.gf(P_chi1chi2_r), levels=(s1_chi1chi2_r,), linewidths=(1,1.5))
  if plot_injection_lines == True:
    plt.axvline(x=chi1_inj, ls='--', color='k')
    plt.axhline(y=chi2_inj, ls='--', color='k')
  plt.xlabel('$\chi _1$')
  ...
  plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_Mfchif_r), levels=(s1_Mfchif_r,), linewidths=(1,1.5))
  if plot_injection_lines == True:
    plt.axvline(x=Mf_inj, ls='--', color='k')
    plt.axhline(y=chif_inj, ls='--', color='k')
  plt.xlabel('$M_f [M_{\odot}]$')
  ...
  plt.contour(m1_bins_imr[:-1], m2_bins_imr[:-1], tgr.gf(P_m1m2_imr), levels=(s1_m1m2_imr,), linewidths=(1,1.5))
  if plot_injection_lines == True:
    plt.axvline(x=m1_inj, ls='--', color='k')
    plt.axhline(y=m2_inj, ls='--', color='k')
  plt.xlabel('$m_1 [M_{\odot}]$')
  ...
  plt.contour(chi1_bins_imr[:-1], chi2_bins_imr[:-1], tgr.gf(P_chi1chi2_imr), levels=(s1_chi1chi2_imr,), linewidths=(1,1.5))
  if plot_injection_lines == True:
    plt.axvline(x=chi1_inj, ls='--', color='k')
    plt.axhline(y=chi2_inj, ls='--', color='k')
  plt.xlabel('$\chi _1$')
  ...
  plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_Mfchif_imr), levels=(s1_Mfchif_imr,), linewidths=(1,1.5))
  if plot_injection_lines == True:
    plt.axvline(x=Mf_inj, ls='--', color='k')
    plt.axhline(y=chif_inj, ls='--', color='k')
  plt.xlabel('$M_f [M_{\odot}]$')
  ...
  # IR overlap
  plt.figure(figsize=(5,5))
  CSi = plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_Mfchif_i), levels=(s1_Mfchif_i,), linewidths=(1,1.5), colors='orange')
  CSr = plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_Mfchif_r), levels=(s1_Mfchif_r,), linewidths=(1,1.5), colors='red')
  CSimr = plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_Mfchif_imr), levels=(s1_Mfchif_imr,), linewidths=(1,1.5), colors='k')
  if plot_injection_lines == True:
    plt.axvline(x=Mf_inj, ls='--', color='k')
    plt.axhline(y=chif_inj, ls='--', color='k')
  ...
  plt.contour(Mf_bins[:-1], chif_bins[:-1], tgr.gf(P_dMfdchif), levels=(s1_dMfdchif,), linewidths=(1,1.5))
  ...
  conf_v1 = confidence(P_eps)
  s1_v1 = conf_v1.height_from_level(0.68)
  s2_v1 = conf_v1.height_from_level(0.95)

  conf_v2 = confidence(P_sig)
  s1_v2 = conf_v2.height_from_level(0.68)
  s2_v2 = conf_v2.height_from_level(0.95)

  # Calculation of condifence edges
  left1_v1 = min(eps_vec[np.where(P_eps>=s1_v1)[0]])
  right1_v1 = max(eps_vec[np.where(P_eps>=s1_v1)[0]])

  left2_v1 = min(eps_vec[np.where(P_eps>=s2_v1)[0]])
  right2_v1 = max(eps_vec[np.where(P_eps>=s2_v1)[0]])

  left1_v2 = min(sig_vec[np.where(P_sig>s1_v2)[0]])
  right1_v2 = max(sig_vec[np.where(P_sig>s1_v2)[0]])

  left2_v2 = min(sig_vec[np.where(P_sig>s2_v2)[0]])
  right2_v2 = max(sig_vec[np.where(P_sig>s2_v2)[0]])

  plt.figure(figsize=(5,5))
  plt.subplot2grid((3,3), (0,0), colspan=2)
  plt.plot(eps_vec, P_eps, color='k', lw=1)
  ...
  plt.pcolormesh(eps_vec,sig_vec,P_eps_sig, cmap='YlOrBr')
  plt.contour(eps_vec,sig_vec,tgr.gf(P_eps_sig), levels=(s1_v1v2,), linewidths=(1,1.5))
  plt.plot(0, 0, 'k+', ms=12, mew=2)
  plt.xlabel('$\Delta M_f/M_f$')
  ...
  plt.savefig('%s/img/epssig.png' %(out_dir), dpi=300)
  plt.savefig('%s/img/epssig_thumb.png' %(out_dir), dpi=72)
